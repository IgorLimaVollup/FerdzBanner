...
<h1>Relatório de ". date('d/m/Y',strtotime("-15 days")) ." à ". date('d/m/Y',strtotime("-1 days")) ." </h1>
<table border='1'>
<tr>
	<th>
		ID do banner
...
for($i=0;$i<count($querybanners->posts);$i++) {
	...
	$resultsViews = $wpdb->get_results( 'SELECT COUNT(*) AS cntviews FROM `wp_ferdzbanner_impressaocount` WHERE date(data_clique) < CURDATE() AND date(data_clique) > SUBDATE(CURDATE(),15) AND id_banner = '.$pstId.';', OBJECT );
	$resultsCliques = $wpdb->get_results( 'SELECT COUNT(*) AS cntcliques FROM `wp_ferdzbanner_viewcount` WHERE date(data_clique) < CURDATE() AND date(data_clique) > SUBDATE(CURDATE(),15) AND id_banner = '.$pstId.';', OBJECT );
...
if(get_option('recebimento_email')) {
	...
	$recebimento = "atendimento@nautica.com.br";
		...
		if(wp_mail( $recebimento,  date('d/m/Y',strtotime("-15 days")) ." à ". date('d/m/Y',strtotime("-1 days")), $mensagem, $headers )) {
