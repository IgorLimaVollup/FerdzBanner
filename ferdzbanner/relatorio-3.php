...
$mensagem.= "
<h1>Relatório do mês de ". strftime('%B', strtotime('-1 month today')) ." </h1>
<table border='1'>
<tr>
	<th>
		ID do banner
...
for($i=0;$i<count($querybanners->posts);$i++) {
	...
	$resultsViews = $wpdb->get_results( 'SELECT COUNT(*) AS cntviews FROM wp_ferdzbanner_impressaocount WHERE YEAR(data_clique) = YEAR(CURRENT_DATE - INTERVAL 1 MONTH) AND MONTH(data_clique) = MONTH(CURRENT_DATE - INTERVAL 1 MONTH) AND id_banner = '.$pstId.';', OBJECT );
	$resultsCliques = $wpdb->get_results( 'SELECT COUNT(*) AS cntcliques FROM wp_ferdzbanner_viewcount WHERE YEAR(data_clique) = YEAR(CURRENT_DATE - INTERVAL 1 MONTH) AND MONTH(data_clique) = MONTH(CURRENT_DATE - INTERVAL 1 MONTH) AND id_banner = '.$pstId.';', OBJECT );
...
if(wp_mail( $recebimento, strftime('%B', strtotime('-1 month today')), $mensagem, $headers )) {
